...
# FINAL ASPECT RATIOS: Includes common screen and photography ratios
...
class FSLGeminiGenerateImage:
    """Gemini image generator with inpainting support, quality, and aspect ratio controls via prompt injection.
    ...
    def INPUT_TYPES(cls) -> Dict[str, Any]:
        tooltip_wh = "These values are used for metadata only. Actual image size is determined by 'aspect_ratio' and API limits (~1K max)."
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True, "default": ""}),
                "model": (MODEL_CHOICES, {"default": "gemini-3-pro-image-preview"}),
                "width": ("INT", {"default": 1024, "min": 64, "max": 2048, "step": 64, "tooltip": tooltip_wh}), 
                "height": ("INT", {"default": 1024, "min": 64, "max": 2048, "step": 64, "tooltip": tooltip_wh}), 
                "aspect_ratio": (ASPECT_CHOICES, {"default": "1:1", "tooltip": "Request this aspect ratio via prompt injection."}),
                "quality": (QUALITY_CHOICES, {"default": "Standard", "tooltip": "Standard requests 1K-style resolution. High requests 4K-style resolution via prompt injection."}),
                "temperature": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 2.0, "step": 0.1}),
                "seed": ("INT", {"default": -1, "min": -1, "max": 0x7FFFFFFF}),
                "enable_safety": ("BOOLEAN", {"default": False, "tooltip": "If true, enables default API safety filters. If false, attempts to disable safety filters (BLOCK_NONE)."}),
    ...
    def generate(
        self,
        prompt: str, model: str, width: int, height: int, aspect_ratio: str, quality: str, temperature: float, seed: int,
        enable_safety: bool, 
        ...
        # --- Quality/Aspect Ratio Injection Logic ---
        if quality == "High":
            res_text = "high-fidelity, 4K resolution image"
        else:
            res_text = "detailed, 1K resolution image" # Standard

        # The new, stronger injection statement, which requests the desired size/aspect ratio
        injection_text = (
            f"\n\n***FOR IMAGE GENERATION:*** Generate this as a **{res_text}** in a **{aspect_ratio}** aspect ratio."
        )

        final_prompt = prompt.strip() + injection_text

        # Build parts
        parts: List[Dict[str, Any]] = []
        is_inpainting = use_mask and (mask is not None or mask_image is not None)
        ...
        # Append the final prompt (with injection) as the main text part
        ...
        if is_inpainting and resolved_base == "none":
        ...
        # Conditionally disable safety settings
        ...
        if not b64_img:
            ...
            raise RuntimeError(f"Failed to find inline image in Gemini response. Finish Reason: {finish_reason}. Submitted Prompt: {final_prompt[:250]}... First 1k of JSON: {str(resp_json)[:1000]}")
        ...
        out_prompt = final_prompt if output_prompt else ""
        metadata = {
            "prompt": prompt, "model": model, "width": width, "height": height, "aspect_ratio": aspect_ratio, "quality": quality,
...
NODE_CLASS_MAPPINGS = {"FSLGeminiGenerateImage": FSLGeminiGenerateImage}
NODE_DISPLAY_NAME_MAPPINGS = {"FSLGeminiGenerateImage": "FSL Gemini Generate Image"}